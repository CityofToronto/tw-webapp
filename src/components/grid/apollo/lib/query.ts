...
import {
  Query, QueryError, SortQuery, TableQueryResult, FieldQuery,
} from '../types';
...
export const getTableFields = ({ tableName, fields }:FieldQuery) => {
  apollo.query({
    query: gql`{
      ${tableName} {
        ${fields}
    ...
    .catch((error:QueryError) => dispatchError);
};

// This is used to query a value list (e.g. TRADE_TYPES) and returns all values
export const getValidTypes = ({ tableName }: Query): Promise<string[]> => apollo.query({
  query: gql`{
      ${tableName} {
        type
      }
    }`,
}).then(response => response.data[tableName].map(x => x.type))
